...
#define IDM_MODE_WAHL 1
#define IDM_MODE_AKT 2
#define IDM_MODE_VORG 3
#define IDM_MODE_EXIT 4
  ...
  case IDM_MODE_WAHL:
    ...
    case IDM_MODE_VORG:
     ...
     case IDM_MODE_EXIT:
       ...
       val= MessageBox(hwnd,"Are you sure?","Wait!",MB_YESNO | MB_ICONEXCLAMATION);
       if(val==IDYES){
        DestroyWindow(hwnd);
       }
        break;
    ...
    case WM_CLOSE:
        MessageBeep(MB_OK);
    if (MessageBox(hwnd, "Really quit?", "My application", MB_OKCANCEL | MB_ICONEXCLAMATION ) == IDOK)
    {
        DestroyWindow(hwnd);
    }
...
void AddMenus(HWND hwnd){
 ...
 // AppendMenu(hSubMenu,MF_STRING,NULL,"Subitems");
    ...
    AppendMenu(hFileMenu,MF_STRING,IDM_MODE_WAHL,"UhrZeit wählen");
     //AppendMenu(hFileMenu,MF_STRING,IDM_MODE_VORG,"Vorgegeben");
    AppendMenu(hFileMenu,MF_SEPARATOR,NULL,NULL);
    AppendMenu(hFileMenu,MF_STRING,IDM_MODE_EXIT,"Exit");

    CheckMenuRadioItem(hMenu, IDM_MODE_AKT, IDM_MODE_EXIT,
		IDM_MODE_AKT, MF_BYCOMMAND);

     AppendMenu(hMenu,MF_POPUP,(UINT_PTR)hFileMenu,"Datei");

    SetMenu(hwnd,hMenu);
}
LRESULT CALLBACK DialogProcedure(HWND fenster,
    UINT msg,
    WPARAM parameter1,
    LPARAM parameter2){
        switch (msg)
        ...
        case WM_CLOSE:
            ...
            break;
        default:
            return DefWindowProcW(fenster,msg,parameter1,parameter2);
        }
 ...
 void registerDialogClass(HINSTANCE hInst){
   ...
   dialog.lpszClassName = L"DigClock";
 ...
 void displayDialog(HWND hwnd){
   ...
   HWND hd=CreateWindowW(L"DigClock",L"Dialog",WS_VISIBLE|WS_OVERLAPPEDWINDOW,400,400,200,200,hwnd,NULL,NULL,NULL);
